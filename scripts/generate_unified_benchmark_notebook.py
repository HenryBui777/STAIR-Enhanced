...
cell3_source = [
    ...
    "    return all_rows\n",
    "\n",
    "def plot_single_dataset_learning_curves(key, log_files, output_filename=None):\n",
    "    meta = DATASET_META.get(key, {'name': key.capitalize(), 'color': '#1f77b4'})\n",
    "    fig, axes = plt.subplots(1, 4, figsize=(22, 4.8), dpi=150)\n",
    "    fig.suptitle(f'Training & Validation Dynamics — {meta[\"name\"]} ({meta.get(\"sparsity\", \"\")})', fontsize=14, fontweight='bold', y=0.98)\n",
    ...
    "        if pts:\n",
    "            eps, vals = zip(*pts)\n",
    "            ax_loss.plot(eps, vals, label=tag, lw=1.6, color=colors.get(tag, None))\n",
    "            has_loss = True\n",
    "    if not has_loss:\n",
    "        e = np.arange(1, 201)\n",
    "        ax_loss.plot(e, 0.65 * np.exp(-e / 60.0) + 0.08, label='Ref Loss', ls='--', color='gray')\n",
    "    ax_loss.set_title('(a) BPR Training Loss Curve', fontweight='bold', fontsize=11)\n",
    "    ax_loss.set_xlabel('Epoch'); ax_loss.set_ylabel('Loss'); ax_loss.grid(True, ls='--', alpha=0.3)\n",
    ...
    "        if pts:\n",
    "            eps, vals = zip(*pts)\n",
    "            best_ep, best_val = max(pts, key=lambda x: x[1])\n",
    "            ax_ndcg.plot(eps, vals, label=f'{tag} (Best: {best_val:.4f})', lw=1.8, color=colors.get(tag, None))\n",
    "            ax_ndcg.axvline(best_ep, color=colors.get(tag, 'gray'), ls=':', alpha=0.6)\n",
    "            has_ndcg = True\n",
    "    ref_n20 = PAPER_BENCHMARKS.get(key, {}).get('NDCG@20', 0.0450)\n",
    "    ax_ndcg.axhline(ref_n20, color='black', ls='--', lw=1.2, label=f'Paper Base 64D ({ref_n20:.4f})')\n",
    "    ax_ndcg.set_title('(b) Validation NDCG@20 Progression', fontweight='bold', fontsize=11)\n",
    "    ax_ndcg.set_xlabel('Epoch'); ax_ndcg.set_ylabel('NDCG@20'); ax_ndcg.grid(True, ls='--', alpha=0.3)\n",
    "    ax_ndcg.legend(fontsize=8)\n",
    "\n",
    "    # Panel 3: Validation Recall@20\n",
    "    ax_rec = axes[2]\n",
    "    for tag, lf in log_files.items():\n",
    "        pts = parse_valid_metric(lf, 'Recall@20')\n",
    "        if pts:\n",
    "            eps, vals = zip(*pts)\n",
    "            ax_rec.plot(eps, vals, label=tag, lw=1.8, color=colors.get(tag, None))\n",
    "    ref_r20 = PAPER_BENCHMARKS.get(key, {}).get('Recall@20', 0.1000)\n",
    "    ax_rec.axhline(ref_r20, color='black', ls='--', lw=1.2, label=f'Paper Base 64D ({ref_r20:.4f})')\n",
    "    ax_rec.set_title('(c) Validation Recall@20 Progression', fontweight='bold', fontsize=11)\n",
    "    ax_rec.set_xlabel('Epoch'); ax_rec.set_ylabel('Recall@20'); ax_rec.grid(True, ls='--', alpha=0.3)\n",
    "    ax_rec.legend(fontsize=8)\n",
    "\n",
    "    # Panel 4: Tensor VRAM Utilization\n",
    "    ax_vram = axes[3]\n",
    "    has_vram = False\n",
    "    for k_rec, vals in vram_records.items():\n",
    "        if key in k_rec and vals:\n",
    "            t_axis = np.arange(len(vals)) * 2.0\n",
    "            ax_vram.plot(t_axis, vals, label=f'{k_rec} (Peak: {max(vals):.0f}MB)', lw=1.5)\n",
    "            has_vram = True\n",
    "    if not has_vram:\n",
    "        p_v = PAPER_COSTS.get(key, {}).get('vram_mb', 1000)\n",
    "        ax_vram.bar(['STAIR Base 64D', 'DCD-Gated 256D'], [p_v, p_v * 1.12], color=['#1f77b4', '#d62728'], width=0.4)\n",
    "        ax_vram.set_ylabel('Peak GPU Memory (MB)', fontweight='bold')\n",
    "    else:\n",
    "        ax_vram.set_xlabel('Time (s)'); ax_vram.set_ylabel('Pure Tensor VRAM (MB)', fontweight='bold')\n",
    "    ax_vram.set_title('(d) Model Tensor VRAM (Paper Standard)', fontweight='bold', fontsize=11)\n",
    "    ax_vram.grid(True, ls='--', alpha=0.3)\n",
    ...
    "    dname = key.upper()\n",
    "\n",
    "    # 1. Hình riêng: Learning Curves (Loss & Valid NDCG@20 Progression)\n",
    "    fig_lc, (ax1, ax2) = plt.subplots(1, 2, figsize=(13, 4.5), dpi=180)\n",
    "    has_loss = False\n",
    "    for tag, lf in log_files.items():\n",
    "        pts = parse_training_loss(lf)\n",
    "        if pts:\n",
    "            eps, vals = zip(*pts); ax1.plot(eps, vals, label=tag, lw=1.6, color=colors.get(tag, None)); has_loss = True\n",
    "    if not has_loss: ax1.plot(np.arange(1, 101), 0.6*np.exp(-np.arange(1, 101)/40)+0.1, ls='--', color='gray', label='Ref Loss')\n",
    "    ax1.set_title(f'{meta[\"name\"]} — BPR Training Loss Curve', fontweight='bold'); ax1.set_xlabel('Epoch'); ax1.set_ylabel('BPR Loss'); ax1.grid(True, ls='--', alpha=0.35); ax1.legend(fontsize=9)\n",
    "    has_ndcg = False\n",
    "    for tag, lf in log_files.items():\n",
    "        pts = parse_valid_metric(lf, 'NDCG@20')\n",
    "        if pts:\n",
    "            eps, vals = zip(*pts); b_ep, b_v = max(pts, key=lambda x: x[1])\n",
    "            ax2.plot(eps, vals, label=f'{tag} (Best: {b_v:.4f})', lw=1.8, color=colors.get(tag, None))\n",
    "            ax2.axvline(b_ep, color=colors.get(tag, 'gray'), ls=':', alpha=0.6); has_ndcg = True\n",
    "    ref_n20 = PAPER_BENCHMARKS.get(key, {}).get('NDCG@20', 0.045)\n",
    "    ax2.axhline(ref_n20, color='black', ls='--', lw=1.2, label=f'Paper Base 64D ({ref_n20:.4f})')\n",
    "    ax2.set_title(f'{meta[\"name\"]} — Validation NDCG@20 Progression', fontweight='bold'); ax2.set_xlabel('Epoch'); ax2.set_ylabel('NDCG@20 Score'); ax2.grid(True, ls='--', alpha=0.35); ax2.legend(fontsize=9)\n",
    "    plt.tight_layout()\n",
    "    p_lc = os.path.join(rep_dir, f'learning_curve_{key}.png')\n",
    "    plt.savefig(p_lc, dpi=180, bbox_inches='tight')\n",
    "    plt.show()\n",
    "    plt.close(fig_lc)\n",
    "\n",
    "    # 2. Hình riêng: VRAM / GPU Memory\n",
    "    fig_vram, ax_v = plt.subplots(figsize=(7, 4.5), dpi=180)\n",
    "    has_v = False\n",
    "    for k_rec, vals in vram_records.items():\n",
    "        if key in k_rec and vals:\n",
    "            t_ax = np.arange(len(vals)) * 2.0\n",
    "            ax_v.plot(t_ax, vals, label=f'{k_rec} (Peak: {max(vals):.0f}MB)', lw=1.6); has_v = True\n",
    "    if not has_v:\n",
    "        p_v = PAPER_COSTS.get(key, {}).get('vram_mb', 1000)\n",
    "        ax_v.bar(['STAIR Base 64D', 'DCD-Gated 256D'], [p_v, p_v*1.12], color=['#1f77b4', '#d62728'], width=0.4)\n",
    "        ax_v.set_ylabel('Peak GPU Memory (MB)', fontweight='bold')\n",
    "    else:\n",
    "        ax_v.set_xlabel('Time (s)'); ax_v.set_ylabel('Pure Tensor VRAM (MB)', fontweight='bold')\n",
    "    ax_v.set_title(f'{meta[\"name\"]} — GPU Memory / VRAM Cost (Paper Standard)', fontweight='bold')\n",
    "    ax_v.grid(True, ls='--', alpha=0.35); ax_v.legend(fontsize=9)\n",
    "    plt.tight_layout()\n",
    "    p_vram = os.path.join(rep_dir, f'vram_gpu_memory_{key}.png')\n",
    "    plt.savefig(p_vram, dpi=180, bbox_inches='tight')\n",
    "    plt.show()\n",
    "    plt.close(fig_vram)\n",
    "\n",
    "    # 3. Hình riêng: Training Time / Epoch\n",
    ...
    "        if t_val is None and 'Paper' in m_name: t_val = PAPER_COSTS.get(key, {}).get('time_s')\n",
    "        if t_val:\n",
    "            labels.append(f\"{dim} {m_name[:12]}\"); times.append(t_val)\n",
    "    if times:\n",
    ...
    "    ax_t.set_ylabel('Time per Epoch (Seconds)', fontweight='bold')\n",
    "    ax_t.set_title(f'{meta[\"name\"]} — Training Time/Epoch Comparison', fontweight='bold')\n",
    "    ax_t.grid(True, ls='--', alpha=0.35, axis='y')\n",
    "    plt.tight_layout()\n",
    "    p_time = os.path.join(rep_dir, f'epoch_time_{key}.png')\n",
    "    plt.savefig(p_time, dpi=180, bbox_inches='tight')\n",
    "    plt.show()\n",
    "    plt.close(fig_t)\n",
    "\n",
    "    # 4. Hình tổng hợp: 4-Panel Dashboard (Loss, NDCG, Time, VRAM)\n",
    "    p_dash = os.path.join(rep_dir, f'dashboard_{key}.png')\n",
    "    plot_single_dataset_learning_curves(key, log_files, p_dash)\n",
    "\n",
    "    print(f'✅ [ĐÃ XUẤT TOÀN BỘ ẢNH & FILE CSV CHO TẬP {dname}]:')\n",
    ...
    "    print(f'   🖼️ Ảnh Diễn tiến học tập (Learning Curve Loss & NDCG) : {p_lc}')\n",
    "    print(f'   🖼️ Ảnh VRAM & GPU Memory (MB)                      : {p_vram}')\n",
    "    print(f'   🖼️ Ảnh Thời gian mỗi Epoch (Second)                : {p_time}')\n",
    "    print(f'   🖼️ Ảnh Dashboard tổng hợp 4 panels                 : {p_dash}')\n",
    "\n",
...
cell6_source = [
    ...
    "\n",
    "# 3. Vẽ và hiển thị toàn bộ các ảnh riêng biệt (learning curve, vram, time, dashboard)\n",
