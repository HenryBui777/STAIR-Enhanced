...
Về hiệu năng phần cứng, kiến trúc STAIR-NE-NLGCL+ (v3) đạt sự ổn định vượt trội trên hạ tầng tính toán thực nghiệm GPU Tesla T4 (16\,GB). Mặc dù gánh thêm toàn bộ nhánh học tương phản InfoNCE, cơ chế Dynamic Slicing $[B \times B]$ kết hợp với việc gạt bỏ lớp MLP projection head đã giúp khống chế chặt chẽ mức tiêu thụ bộ nhớ: trên tập dữ liệu khổng lồ Amazon Electronics (~1.7 triệu tương tác, catalog 63,001 items), VRAM đỉnh chỉ dừng lại ở mức 2785\,MB (~2.72\,GB). Mức tiêu thụ này chỉ chiếm 18.1\% tổng dung lượng 16\,GB của GPU, loại bỏ 100\% nguy cơ tràn bộ nhớ (Out-of-Memory) và duy trì đường tiêu thụ VRAM phẳng tuyệt đối suốt 500 epochs liên tục (~6.01 giờ). Trên hai tập dữ liệu còn lại, mức tiêu thụ VRAM đỉnh cũng được kiểm soát an toàn dưới ngưỡng 1.2\,GB (đạt 925.2\,MB trên Baby và 1141\,MB trên Sports, chỉ tăng nhẹ từ 17\% đến 21\% so với baseline BPR thuần để phục vụ nhánh tương phản đồ thị). Sự ổn định này khẳng định tính khả thi và khả năng mở rộng mạnh mẽ của mô hình trên các hạ tầng máy học công nghiệp thực tế.
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item STAIR-NE-NLGCL+ (v3) kiểm soát bộ nhớ xuất sắc nhờ kỹ thuật Dynamic Slicing $[B \times B]$: khống chế VRAM đỉnh trên tập khổng lồ Electronics chỉ ở mức 2785\,MB (~2.72\,GB, tương đương 18.1\% trần GPU 16\,GB Tesla T4) và dưới 1.2\,GB trên Baby/Sports, loại bỏ hoàn toàn nguy cơ OOM trong khi thiết lập kỷ lục hiệu năng mới.
